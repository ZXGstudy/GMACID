...
def process_fasta_file(input_file, output_file, interval=10):
        ...
        for line in infile:
            ...
            if line.startswith('>'):
                ...
                if current_header and current_sequence:
                    ...
                    length = len(sequence)
                    for start in range(0, length - 20, interval):
                        substr = sequence[start:start+20]
                        outfile.write(f">{current_header}_slice_{start+1}_to_{start+20}\n")
...
def process_fastq_file(input_file, output_file, interval=10):
    with open(input_file, 'r') as infile, open(output_file, 'w') as outfile:
        while True:
            ...
            for start in range(0, length - 20, interval):
                substr = sequence[start:start+20]
                outfile.write(f">{header[1:]}_slice_{start+1}_to_{start+20}\n")
                outfile.write(f"{substr}\n")
...
# Example usage
input_folder = '/home/zxg/GMACID/hostDNA'
output_folder = '/home/zxg/GMACID/hostDNA/cutkmer_no_remove'
